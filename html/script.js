...
$(document).ready(function()
{
    $("#inboxList").on("click",'li',function(event)
    {
        ...
        $.post('https://smdx-telegram/getview', JSON.stringify({id: $(this).data('id')}));
        $(".inbox").fadeOut().hide();
        $(".view").fadeIn().show();
    });
});
...
{
    $.post('https://smdx-telegram/NUIFocusOff', JSON.stringify({}));
});

// delete message
$(".telegram_delete_button").click(function(event)
{
    $.post('https://smdx-telegram/delete', JSON.stringify({id: itemToDel}));
    $.post('https://smdx-telegram/NUIFocusOff', JSON.stringify({}));
});